...
su jenkins -c "curl --silent -L https://get.rvm.io | bash"
...
for RUBY in $RUBIES
  do
    su jenkins -c "source ~jenkins/.bash_profile && rvm install $RUBY"
done
# http://stackoverflow.com/questions/9336596/rvm-installation-not-working-rvm-is-not-a-function
su jenkins -c "source ~jenkins/.bash_profile && bash -lc 'rvm use 2.0.0 --default'"
